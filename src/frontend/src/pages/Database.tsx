...
import { Database as DbIcon, Edit2, Trash2, Plus, LayoutList, Bot, Phone, MapPin, Calendar, Loader2 } from 'lucide-react';
...
export default function Database() {
  ...
  const handleDelete = async (ticket_id: string) => {
    if (!window.confirm(`Are you sure you want to delete ticket ${ticket_id}?`)) return;
    try {
  ...
  const handleEditStatus = async (ticket_id: string, currentStatus: string) => {
    const input = window.prompt("Enter new status (Open, In Progress, Resolved):", currentStatus);
    if (!input) return;
    
    const validStatuses = ['Open', 'In Progress', 'Resolved'];
    const formattedInput = input.trim();
    
    const newStatus = validStatuses.find(s => s.toLowerCase() === formattedInput.toLowerCase());
    ...
    try {
      ...
      setTickets((prev) => prev.map((t) => t.ticket_id === ticket_id ? { ...t, status: newStatus } : t));
    } catch (error) {
  ...
  const filteredTickets = tickets
    ...
    .sort((a, b) => {
      ...
      if (sortBy === 'Priority') {
        const pScores: Record<string, number> = { 'EXTREME': 4, 'STRONG': 3, 'MEDIUM': 2, 'LOW': 1, 'P1': 4, 'P2': 3, 'P3': 2, 'P4': 1 };
        return (pScores[b.priority?.toUpperCase()] || 0) - (pScores[a.priority?.toUpperCase()] || 0);
      }
      return 0;
    });
  ...
  const getPriorityStyle = (priority: string) => {
    switch (priority?.toUpperCase()) {
      case 'EXTREME': 
      case 'P1': return 'bg-red-50 text-red-600 border-red-200 shadow-sm shadow-red-500/10';
      case 'STRONG': 
      case 'P2': return 'bg-orange-50 text-orange-600 border-orange-200 shadow-sm shadow-orange-500/10';
      case 'MEDIUM': 
      case 'P3': return 'bg-amber-50 text-amber-600 border-amber-200 shadow-sm shadow-amber-500/10';
  ...
  return (
    <div className="flex flex-col relative reveal min-h-[calc(100vh-7rem)] pb-20">
      ...
      {/* Central Header */}
      <div className="w-full max-w-7xl mx-auto px-6 flex flex-col items-center text-center mt-12 mb-16">
        <div className="w-16 h-16 rounded-3xl flex items-center justify-center bg-gradient-to-br from-emerald-400 to-teal-500 shadow-xl shadow-emerald-500/20 mb-6">
        ...
        <h2 className="text-[24px] sm:text-[32px] font-medium text-slate-400 tracking-tight leading-tight">
          Manage and track all reported field operations
        </h2>
        
        <div className="mt-8">
          <button className="flex items-center gap-2.5 px-6 py-3 rounded-full bg-white/60 border border-slate-200/80 text-slate-700 text-[14px] font-bold hover:bg-white hover:text-emerald-600 hover:shadow-md hover:border-emerald-200 transition-all shadow-sm backdrop-blur-sm">
        ...
        <section className="reveal reveal-d1">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-5 px-4">
            <div className="flex items-center gap-2.5">
              ...
              <h3 className="text-[14px] font-bold text-slate-500 uppercase tracking-[0.15em]">AI-Triaged Tickets</h3>
              {isLoading && <Loader2 className="w-4 h-4 text-slate-400 animate-spin ml-2" />}
            </div>
              ...
              <table className="w-full text-left min-w-[1200px]">
                <thead>
                  <tr className="text-[11px] text-slate-400 uppercase tracking-widest border-b border-slate-100 bg-slate-50/50">
                    ...
                    <th className="px-6 py-5 font-bold">Reporter & Location</th>
                    <th className="px-6 py-5 font-bold">Incident Summary</th>
                    <th className="px-6 py-5 font-bold">AI Triage Analysis</th>
                ...
                <tbody className="divide-y divide-slate-100/80">
                  {filteredTickets.length === 0 ? (
                    <tr>
                      <td colSpan={6} className="px-6 py-12 text-center text-slate-400 text-[14px] font-medium">
                    ...
                    <tr key={t.ticket_id} className="hover:bg-slate-50/50 transition-colors">
                      ...
                      <td className="px-6 py-6 align-top">
                        <div className="flex flex-col gap-1.5">
                          <span className="text-[13px] font-extrabold text-slate-700 font-mono bg-slate-100 px-2.5 py-1 rounded-md w-fit border border-slate-200/80 shadow-sm">{t.ticket_id}</span>
                      ...
                      <td className="px-6 py-6 align-top">
                        <div className="flex flex-col gap-3">
                          <div>
                            <p className="text-[14px] font-bold text-text-primary">{t.raised_by}</p>
                          ...
                          <div>
                            <p className="text-[13px] font-bold text-slate-600">{t.village}, {t.district}</p>
                        ...
                        <div className="flex items-start gap-3 bg-white border border-slate-200 shadow-sm rounded-xl p-3.5">
                          ...
                          <div className="flex flex-col gap-1.5">
                            <div className="flex items-center gap-2">
                              <span className="text-[12px] font-bold text-text-primary bg-slate-100 px-1.5 rounded border border-slate-200">Score: {t.priority_score}</span>
                              <span className="text-[10px] font-bold text-slate-400 tracking-wide uppercase">LLM: {t.llm_score} • Rule: {t.rule_score}</span>
                            </div>
                            <p className="text-[12px] font-medium text-slate-500 italic leading-snug">"{t.llm_reason}"</p>
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-6 align-top">
                        <div className="flex flex-col gap-2.5 items-start">
                          <span className={`px-3 py-1.5 rounded-lg text-[11px] font-bold uppercase tracking-wider border ${getPriorityStyle(t.priority)}`}>
                            {t.priority}
                          </span>
                          <span className="inline-flex items-center gap-2 text-[12px] font-bold text-slate-600 bg-white px-3 py-1 rounded-full border border-slate-200 shadow-sm">
                            <span className={`w-2 h-2 rounded-full shadow-sm ${getStatusStyle(t.status)}`} />
                            {t.status}
                          </span>
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-6 align-top text-right">
                        <div className="flex items-center justify-end gap-1 opacity-40 hover:opacity-100 transition-opacity">
                          <button 
                            onClick={() => handleEditStatus(t.ticket_id, t.status)}
                            title="Edit Status"
